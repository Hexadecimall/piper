...
fn bundled_introspection_context_and_logging_modules_execute() {
    ...
    write(base.join("main.py"), "import ast\nimport contextvars\nimport decimal\nimport hashlib\nimport logging\nimport time\nvalue = contextvars.ContextVar('value', default=3)\ntoken = value.set(9)\nprint(value.get(), contextvars.copy_context()[value])\nvalue.reset(token)\nprint(value.get())\nnode = ast.Constant(value=42)\nprint(isinstance(node, ast.AST), node.value, ast.dump(node))\nprint(hasattr(logging, 'getLogger'), len(time.strftime('%Y', time.gmtime(0))))\nprint(decimal.Decimal('1.25') + decimal.Decimal('2.75'))\nprint(decimal.Decimal('1.20') * decimal.Decimal('3'))\ndecimal.getcontext().prec = 6\nprint(decimal.Decimal('1') / decimal.Decimal('7'))\nprint(decimal.Decimal('2.345').quantize(decimal.Decimal('0.01')))\nprint(decimal.Decimal('2.355').quantize(decimal.Decimal('0.01')))\nprint(decimal.Decimal('-7.5') // decimal.Decimal('2'), decimal.Decimal('-7.5') % decimal.Decimal('2'))\nprint(decimal.Decimal('1.2300').normalize(), decimal.Decimal('2') ** decimal.Decimal('5'))\nprint(decimal.Decimal('1.2') < decimal.Decimal('1.20'), int(decimal.Decimal('-12.9')), round(decimal.Decimal('2.5')), round(decimal.Decimal('3.5')))\nprint(hashlib.sha256(b'abc').hexdigest())\nprint(hashlib.sha224(b'abc').hexdigest())\n");
    let executable = base.join("program");
    piper::compile_file(&base.join("main.py"), &executable, &Default::default()).unwrap();
    let output = Command::new(&executable).output().unwrap();
    assert!(output.status.success(), "{}", String::from_utf8_lossy(&output.stderr));
